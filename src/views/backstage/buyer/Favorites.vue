        ...
        <div class="infinite-list-wrapper" v-if="isMounted && rowCount != 0">
            <div v-infinite-scroll="load" class="list" :infinite-scroll-disabled="disabled"
                ...
                <el-row v-for="i in rowCount" class="list-item" :gutter="5">
                    <el-col v-for="userFavorite in userFavorites.slice((i - 1) * 4, i * 4)" :key="userFavorite.fid"
                        :span="6">
                        <router-link :to="{ name: 'commodity', params: { cid: userFavorite.commodity.cid } }" class="a">
                            <el-card :body-style="{ padding: '.3rem' }" shadow="hover">
                                ...
                                <div style="padding: 14px">
                                    ...
                                    <div class="bottom">
                                        <time class="collect-time">{{ userFavorite.time }}</time>
                                    </div>
...
onMounted(() => {
    ...
    pathStore.path = [
        ...
        { name: "我的收藏", path: '/buyer/favorites' },
    ]
})
</script>
